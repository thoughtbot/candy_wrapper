...
export const Checkbox = ({ type: _type, includeHidden, uncheckedValue, errorKey, label, value, ...rest }) => {
...
export const CollectionRadioButtons = ({ includeHidden, collection, label, errorKey, }) => {
    ...
    return (<div className="grid gap-2">
      {includeHidden && (<input type="hidden" name={name} defaultValue={''} autoComplete="off"/>)}
      <Label>{label}</Label>
      <RadioGroup name={name} defaultValue={defaultItem?.value} value={item?.value}>
        {collection.map((option) => (<div className="flex items-center gap-2" key={option.value}>
...
export const RangeField = ({ type: _type, label, errorKey, name, value, defaultValue, min, max, ...rest }) => {
    ...
    return (<div className="grid gap-2">
      <Label>{label}</Label>
      <Slider name={name} value={numValue !== undefined ? [numValue] : undefined} defaultValue={numDefault !== undefined ? [numDefault] : [numMin]} min={numMin} max={numMax} {...rest}/>
      <FieldError errorKey={errorKey}/>
    </div>);
};
...
export const Select = ({ includeHidden, name, options, errorKey, type: _type, ...rest }) => {
    if ('multiple' in rest && rest.multiple) {
        ...
        <select name={name} id={id} multiple={multiple} defaultValue={defaultvalue} value={value} {...nativeRest}>
          {optionElements}
        </select>
        <FieldError errorKey={errorKey}/>
      </div>);
    ...
    return (<div className="grid gap-2">
      <Label htmlFor={id}>{label}</Label>
      <ShadcnSelect name={name} defaultValue={defaultvalue} value={value} {...selectRest}>
        <SelectTrigger id={id}>
          <SelectValue />
        </SelectTrigger>
        <SelectContent>
          {hasGroups
            ? options.map((item) => {
                if ('options' in item) {
                    return (<SelectGroup key={item.label}>
                      <SelectLabel>{item.label}</SelectLabel>
                      {item.options.map((opt) => (<SelectItem key={opt.value} value={opt.value} disabled={opt.disabled}>
                          {opt.label}
                        </SelectItem>))}
                    </SelectGroup>);
                }
