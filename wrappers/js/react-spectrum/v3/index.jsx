...
import { TextField as SpectrumTextField, TextArea as SpectrumTextArea, NumberField as SpectrumNumberField, DateField as SpectrumDateField, TimeField as SpectrumTimeField, DatePicker as SpectrumDatePicker, SearchField as SpectrumSearchField, Checkbox as SpectrumCheckbox, CheckboxGroup as SpectrumCheckboxGroup, RadioGroup as SpectrumRadioGroup, Radio as SpectrumRadio, Picker as SpectrumPicker, Item, Section, Slider as SpectrumSlider, Button as SpectrumButton, Form as SpectrumForm, FileTrigger as SpectrumFileTrigger, } from '@adobe/react-spectrum';
...
export const Checkbox = ({ type: _type, includeHidden, uncheckedValue, errorKey, label, checked, defaultChecked, ...rest }) => {
    ...
    return (<>
      ...
      <SpectrumCheckbox name={name} value={rest.value} isSelected={isSelected} defaultSelected={defaultSelected} isInvalid={!!errorMessage}>
        {label}
      </SpectrumCheckbox>
    </>);
};
...
export const TextField = ({ type: _type, label, errorKey, ...rest }) => {
    const errorMessage = useErrorMessage(errorKey);
    return (<SpectrumTextField label={label} name={rest.name} defaultValue={rest.defaultValue} value={rest.value} maxLength={rest.maxLength} validationState={errorMessage ? 'invalid' : undefined} errorMessage={errorMessage}/>);
};
export const EmailField = ({ type: _type, label, errorKey, ...rest }) => {
    const errorMessage = useErrorMessage(errorKey);
    return (<SpectrumTextField label={label} name={rest.name} defaultValue={rest.defaultValue} value={rest.value} type="email" maxLength={rest.maxLength} validationState={errorMessage ? 'invalid' : undefined} errorMessage={errorMessage}/>);
};
export const ColorField = ({ type: _type, label, errorKey, ...rest }) => {
    const errorMessage = useErrorMessage(errorKey);
    return (<SpectrumTextField label={label} name={rest.name} defaultValue={rest.defaultValue} value={rest.value} validationState={errorMessage ? 'invalid' : undefined} errorMessage={errorMessage}/>);
};
export const DateField = ({ type: _type, label, errorKey, value, defaultValue, min, max, ...rest }) => {
    ...
    return (<SpectrumDateField label={label} name={rest.name} {...valueProps} {...minMaxProps} validationState={errorMessage ? 'invalid' : undefined} errorMessage={errorMessage}/>);
};
export const DateTimeLocalField = ({ type: _type, label, errorKey, value, defaultValue, min, max, ...rest }) => {
    ...
    return (<SpectrumDatePicker label={label} name={rest.name} granularity="second" {...valueProps} {...minMaxProps} validationState={errorMessage ? 'invalid' : undefined} errorMessage={errorMessage}/>);
};
export const TimeField = ({ type: _type, label, errorKey, value, defaultValue, ...rest }) => {
    ...
    return (<SpectrumTimeField label={label} name={rest.name} {...valueProps} validationState={errorMessage ? 'invalid' : undefined} errorMessage={errorMessage}/>);
};
export const SearchField = ({ type: _type, label, errorKey, autosave: _autosave, results: _results, onsearch: _onsearch, incremental: _incremental, ...rest }) => {
    const errorMessage = useErrorMessage(errorKey);
    return (<SpectrumSearchField label={label} name={rest.name} defaultValue={rest.defaultValue} value={rest.value} validationState={errorMessage ? 'invalid' : undefined} errorMessage={errorMessage}/>);
};
export const TelField = ({ type: _type, label, errorKey, ...rest }) => {
    const errorMessage = useErrorMessage(errorKey);
    return (<SpectrumTextField label={label} name={rest.name} defaultValue={rest.defaultValue} value={rest.value} type="tel" maxLength={rest.maxLength} validationState={errorMessage ? 'invalid' : undefined} errorMessage={errorMessage}/>);
};
export const UrlField = ({ type: _type, label, errorKey, ...rest }) => {
    const errorMessage = useErrorMessage(errorKey);
    return (<SpectrumTextField label={label} name={rest.name} defaultValue={rest.defaultValue} value={rest.value} type="url" maxLength={rest.maxLength} validationState={errorMessage ? 'invalid' : undefined} errorMessage={errorMessage}/>);
};
...
export const NumberField = ({ type: _type, label, errorKey, value, defaultValue, min, max, ...rest }) => {
    ...
    return (<SpectrumNumberField label={label} name={rest.name} {...valueProps} minValue={min} maxValue={max} validationState={errorMessage ? 'invalid' : undefined} errorMessage={errorMessage}/>);
};
export const RangeField = ({ type: _type, label, errorKey, value, defaultValue, ...rest }) => {
    ...
    return (<>
      <SpectrumSlider label={label} defaultValue={numericDefault} value={numericValue}/>
...
export const PasswordField = ({ type: _type, label, errorKey, ...rest }) => {
    const errorMessage = useErrorMessage(errorKey);
    return (<SpectrumTextField label={label} name={rest.name} defaultValue={rest.defaultValue} value={rest.value} type="password" maxLength={rest.maxLength} validationState={errorMessage ? 'invalid' : undefined} errorMessage={errorMessage}/>);
};
export const Select = ({ includeHidden, name, id, options, label, errorKey, multiple, type: _type, ...rest }) => {
    ...
    return (<>
      ...
      <SpectrumPicker label={label} name={name} id={id} selectedKey={selectedKey} defaultSelectedKey={defaultKey} isInvalid={!!errorMessage} errorMessage={errorMessage}>
        {hasGroups
            ? options.map((item) => {
                if ('options' in item) {
                    return (<Section key={item.label} title={item.label}>
                    {item.options.map((opt) => (<Item key={opt.value} textValue={opt.label}>
                        {opt.label}
                      </Item>))}
                  </Section>);
                }
                return (<Item key={item.value} textValue={item.label}>
                  {item.label}
                </Item>);
            })
            : options.map((item) => {
...
export const TextArea = ({ type: _type, label, errorKey, ...rest }) => {
    const errorMessage = useErrorMessage(errorKey);
    return (<SpectrumTextArea label={label} name={rest.name} defaultValue={rest.defaultValue} value={rest.value} validationState={errorMessage ? 'invalid' : undefined} errorMessage={errorMessage}/>);
};
