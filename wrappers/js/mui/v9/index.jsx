...
export const TextFieldComponent = ({ type: _type, label, errorKey, ...rest }) => {
...
export const ColorField = ({ type: _type, color: _color, label, errorKey, ...rest }) => {
    const errorMessage = useErrorMessage(errorKey);
    return (<TextField label={label} type="color" error={!!errorMessage} helperText={errorMessage} {...rest}/>);
};
...
export const NumberField = ({ type: _type, label, errorKey, min, max, step, ...rest }) => {
    const errorMessage = useErrorMessage(errorKey);
...
export const CollectionCheckboxes = ({ includeHidden, collection, label, errorKey, }) => {
    ...
    const checkboxes = collection.map((options) => {
        const { label: checkboxLabel, type: _type, includeHidden: _includeHidden, uncheckedValue: _uncheckedValue, ...rest } = options;
...
export const CollectionRadioButtons = ({ includeHidden, collection, label, errorKey, }) => {
    ...
    const radioButtons = collection.map((options) => {
        const { label: radioLabel, checked: _checked, defaultChecked: _defaultChecked, type: _type, ...rest } = options;
...
export const FileField = ({ type: _type, color: _color, size: _size, label, errorKey, ...rest }) => {
    const errorMessage = useErrorMessage(errorKey);
