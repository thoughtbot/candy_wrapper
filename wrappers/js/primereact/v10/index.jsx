...
export const Checkbox = ({ type: _type, includeHidden, uncheckedValue, errorKey, label, checked = false, ...rest }) => {
    const { name, id } = rest;
    return (<FieldWrapper label={label} id={id} errorKey={errorKey}>
...
export const CollectionCheckboxes = ({ includeHidden, collection, label, errorKey, }) => {
    ...
    const checkboxes = collection.map((options) => {
        const { label: optionLabel, type: _type, includeHidden: _includeHidden, uncheckedValue: _uncheckedValue, checked = false, ...rest } = options;
        return (<div key={rest.id} className="field-checkbox">
...
export const CollectionRadioButtons = ({ includeHidden, collection, label, errorKey, }) => {
    ...
    const radioButtons = collection.map((options) => {
        ...
        return (<div key={rest.value} className="field-radiobutton">
...
export const TextField = ({ type: _type, label, errorKey, ...rest }) => {
    const errorMessage = useErrorMessage(errorKey);
    return (<FieldWrapper label={label} id={rest.id} errorKey={errorKey}>
      <InputText {...rest} type="text" className={errorMessage ? 'p-invalid' : undefined}/>
    </FieldWrapper>);
};
export const EmailField = ({ type: _type, label, errorKey, ...rest }) => {
    const errorMessage = useErrorMessage(errorKey);
    return (<FieldWrapper label={label} id={rest.id} errorKey={errorKey}>
      <InputText {...rest} type="email" className={errorMessage ? 'p-invalid' : undefined}/>
    </FieldWrapper>);
};
export const SearchField = ({ type: _type, label, errorKey, ...rest }) => {
    const errorMessage = useErrorMessage(errorKey);
    return (<FieldWrapper label={label} id={rest.id} errorKey={errorKey}>
      <InputText {...rest} type="search" className={errorMessage ? 'p-invalid' : undefined}/>
    </FieldWrapper>);
};
export const TelField = ({ type: _type, label, errorKey, ...rest }) => {
    const errorMessage = useErrorMessage(errorKey);
    return (<FieldWrapper label={label} id={rest.id} errorKey={errorKey}>
      <InputText {...rest} type="tel" className={errorMessage ? 'p-invalid' : undefined}/>
    </FieldWrapper>);
};
export const UrlField = ({ type: _type, label, errorKey, ...rest }) => {
    const errorMessage = useErrorMessage(errorKey);
    return (<FieldWrapper label={label} id={rest.id} errorKey={errorKey}>
      <InputText {...rest} type="url" className={errorMessage ? 'p-invalid' : undefined}/>
    </FieldWrapper>);
};
export const PasswordField = ({ type: _type, label, errorKey, ...rest }) => {
    const errorMessage = useErrorMessage(errorKey);
    return (<FieldWrapper label={label} id={rest.id} errorKey={errorKey}>
      <Password {...rest} inputId={rest.id} className={errorMessage ? 'p-invalid' : undefined} feedback={false}/>
    </FieldWrapper>);
};
...
export const DateField = ({ type: _type, label, errorKey, value, defaultValue, min, max, ...rest }) => {
    ...
    return (<FieldWrapper label={label} id={rest.id} errorKey={errorKey}>
      <Calendar {...rest} inputId={rest.id} {...valueProps} {...minMaxProps} dateFormat="yy-mm-dd" className={errorMessage ? 'p-invalid' : undefined}/>
    </FieldWrapper>);
};
export const DateTimeLocalField = ({ type: _type, label, errorKey, value, defaultValue, min, max, ...rest }) => {
    ...
    return (<FieldWrapper label={label} id={rest.id} errorKey={errorKey}>
      <Calendar {...rest} inputId={rest.id} showTime {...valueProps} {...minMaxProps} className={errorMessage ? 'p-invalid' : undefined}/>
    </FieldWrapper>);
};
export const TimeField = ({ type: _type, label, errorKey, value, defaultValue, ...rest }) => {
    ...
    return (<FieldWrapper label={label} id={rest.id} errorKey={errorKey}>
      <Calendar {...rest} inputId={rest.id} timeOnly {...valueProps} className={errorMessage ? 'p-invalid' : undefined}/>
    </FieldWrapper>);
};
export const MonthField = ({ type: _type, label, errorKey, value, defaultValue, min, max, ...rest }) => {
    const errorMessage = useErrorMessage(errorKey);
    const valueProps = {};
    if (value) {
        valueProps.value = parseDate(value);
    ...
    return (<FieldWrapper label={label} id={rest.id} errorKey={errorKey}>
      <Calendar {...rest} inputId={rest.id} view="month" dateFormat="yy-mm" {...valueProps} {...minMaxProps} className={errorMessage ? 'p-invalid' : undefined}/>
    </FieldWrapper>);
};
export const NumberField = ({ type: _type, label, errorKey, value, defaultValue, min, max, ...rest }) => {
    const errorMessage = useErrorMessage(errorKey);
    const numericValue = value !== undefined ? Number(value) : defaultValue !== undefined ? Number(defaultValue) : undefined;
    return (<FieldWrapper label={label} id={rest.id} errorKey={errorKey}>
      <InputNumber {...rest} inputId={rest.id} value={numericValue} min={min} max={max} useGrouping={false} className={errorMessage ? 'p-invalid' : undefined}/>
    </FieldWrapper>);
};
export const RangeField = ({ type: _type, label, errorKey, value, defaultValue, ...rest }) => {
    const initialValue = value !== undefined ? Number(value) : defaultValue !== undefined ? Number(defaultValue) : undefined;
    const [sliderValue, setSliderValue] = useState(initialValue);
    return (<FieldWrapper label={label} id={rest.id} errorKey={errorKey}>
      <Slider {...rest} value={sliderValue} onChange={(e) => setSliderValue(e.value)}/>
      <input type="hidden" name={rest.name} value={sliderValue ?? ''}/>
    </FieldWrapper>);
};
export const Select = ({ includeHidden, name, id, options, errorKey, type: _type, ...rest }) => {
    const errorMessage = useErrorMessage(errorKey);
    ...
    if (rest.multiple) {
        ...
        return (<FieldWrapper label={rest.label} id={id} errorKey={errorKey}>
        {addHidden && (<input type="hidden" name={name} value={''} autoComplete="off"/>)}
        <PrimeMultiSelect {...multiRest} inputId={id} options={flatOptions} value={values} onChange={(e) => setValues(e.value)} className={errorMessage ? 'p-invalid' : undefined}/>
    ...
    return (<FieldWrapper label={rest.label} id={id} errorKey={errorKey}>
      <PrimeDropdown {...singleRest} inputId={id} name={name} options={flatOptions} value={selectedValue} className={errorMessage ? 'p-invalid' : undefined}/>
    </FieldWrapper>);
};
export const TextArea = ({ type: _type, label, errorKey, ...rest }) => {
    const errorMessage = useErrorMessage(errorKey);
    return (<FieldWrapper label={label} id={rest.id} errorKey={errorKey}>
      <InputTextarea {...rest} className={errorMessage ? 'p-invalid' : undefined}/>
    </FieldWrapper>);
};
