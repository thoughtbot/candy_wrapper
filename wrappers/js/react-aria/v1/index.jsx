...
export const dateFieldToRACProps = (props) => {
    ...
    const racProps = { ...rest };
    if (max)
        racProps.maxValue = parseDate(max);
    if (min)
        racProps.minValue = parseDate(min);
    if (defaultValue)
        racProps.defaultValue = parseDate(defaultValue);
    if (value)
        racProps.value = parseDate(value);
    return racProps;
};
export const dateTimeLocalFieldToRACProps = (props) => {
    ...
    if (max)
        racProps.maxValue = parseDateTime(max);
    if (min)
        racProps.minValue = parseDateTime(min);
    if (defaultValue)
        racProps.defaultValue = parseDateTime(defaultValue);
    if (value)
        racProps.value = parseDateTime(value);
    return racProps;
};
export const timeFieldToRACProps = (props) => {
    ...
    const racProps = { ...rest };
    if (value)
        racProps.value = parseTime(value);
    if (defaultValue)
        racProps.defaultValue = parseTime(defaultValue);
    if (min)
        racProps.minValue = parseTime(min);
    if (max)
        racProps.maxValue = parseTime(max);
    return racProps;
};
export const Checkbox = ({ includeHidden, uncheckedValue, errorKey, label, ...rest }) => {
    ...
    return (<FormValidationContext.Provider value={validationErrors}>
...
export const CollectionCheckboxes = ({ includeHidden, collection, label, errorKey, }) => {
    ...
    return (<FormValidationContext.Provider value={validationErrors}>
      ...
      <AriaCheckboxGroup {...valueProps} isInvalid={!!errorMessage}>
        ...
        {collection.map(({ checked: _checked, defaultChecked: _defaultChecked, ...checkboxProps }) => (<AriaCheckbox key={checkboxProps.id} name={name} value={checkboxProps.value} id={checkboxProps.id}>
            <div className="checkbox">
              <svg viewBox="0 0 18 18" aria-hidden="true">
                <polyline points="1 9 7 14 15 4"/>
              </svg>
...
export const CollectionRadioButtons = ({ includeHidden, collection, label, errorKey, }) => {
    ...
    return (<FormValidationContext.Provider value={validationErrors}>
      ...
      <AriaRadioGroup name={name} {...valueProps} isInvalid={!!errorMessage}>
        <AriaLabel>{label}</AriaLabel>
        {collection.map(({ label: optionLabel, type: _type, ...radioProps }) => (<AriaRadio key={radioProps.value} value={radioProps.value} id={radioProps.id}>
            {optionLabel}
          </AriaRadio>))}
        {errorMessage && <AriaFieldError>{errorMessage}</AriaFieldError>}
      </AriaRadioGroup>
...
export const TextField = ({ label, errorKey, ...rest }) => {
    ...
    return (<TextBase errorKey={errorKey} {...racProps}>
      <AriaLabel>{label}</AriaLabel>
      <AriaInput />
      <AriaFieldError />
    </TextBase>);
...
export const EmailField = ({ label, errorKey, ...rest }) => {
    ...
    return (<TextBase errorKey={errorKey} {...props} type="email">
      <AriaLabel>{label}</AriaLabel>
      <AriaInput />
      <AriaFieldError />
    </TextBase>);
...
export const SearchField = ({ label, errorKey, type: _type, autosave: _autosave, results: _results, onsearch: _onsearch, incremental: _incremental, ...rest }) => {
    const validationErrors = useErrorKeyValidation({ errorKey, name: rest.name });
    return (<FormValidationContext.Provider value={validationErrors}>
...
export const TelField = ({ label, errorKey, ...rest }) => {
    const { type: _type, ...props } = rest;
    return (<TextBase errorKey={errorKey} {...props} type="tel">
      <AriaLabel>{label}</AriaLabel>
      <AriaInput />
      <AriaFieldError />
    </TextBase>);
...
export const UrlField = ({ label, errorKey, ...rest }) => {
    const { type: _type, ...props } = rest;
    return (<TextBase errorKey={errorKey} {...props} type="url">
      <AriaLabel>{label}</AriaLabel>
      <AriaInput />
      <AriaFieldError />
    </TextBase>);
...
export const MonthField = ({ label, errorKey, type: _type, ...rest }) => {
    return (<TextBase errorKey={errorKey} {...rest}>
      <AriaLabel>{label}</AriaLabel>
...
export const RangeField = ({ label, errorKey, value, defaultValue, min, max, type: _type, ...rest }) => {
    const errorMessage = useErrorMessage(errorKey);
...
export const PasswordField = ({ label, errorKey, ...rest }) => {
    const { type: _type, ...props } = rest;
    return (<TextBase errorKey={errorKey} {...props} type="password">
      <AriaLabel>{label}</AriaLabel>
      <AriaInput />
      <AriaFieldError />
    </TextBase>);
